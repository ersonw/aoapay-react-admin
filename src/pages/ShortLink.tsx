...
import { setClientAdd, setShortLinkDelete, setShortLinkDeleteAll, getShortLink } from '../service';
import moment from 'moment/moment';

class ShortLink extends React.Component<any> {
    state = {
    ...
    onDelete(record: any) {
        ...
        Modal.confirm({
            ...
            title: `确定要删除?`,
            content: `短链ID:${record.shortLink}`,
            onOk: () => {
                setShortLinkDelete({ id: record.id }).then((_) => {
                    let { data } = this.state;
                    this.setState({ data: [] });
    ...
    onDeleteAll() {
        ...
        Modal.confirm({
            ...
            content: '',
            onOk: () => {
                setShortLinkDeleteAll(this.state.selectedRowKeys).then((_) => {
